...
public class TemplateController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<TemplateResponse> update(@AuthenticationPrincipal UserPrincipal principal,
            @PathVariable String id,
            @Valid @RequestBody TemplateRequest request) {
        return ResponseEntity.ok(templateService.update(principal.userId(), id, request));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Void> delete(@AuthenticationPrincipal UserPrincipal principal,
            @PathVariable String id) {
        templateService.delete(principal.userId(), id);
